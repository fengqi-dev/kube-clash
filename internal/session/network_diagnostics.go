...
func inspectNetwork(discovery cluster.Discovery) *NetworkDiagnostics {
	...
	if err != nil {
		diagnostics.Issues = append(diagnostics.Issues, NetworkDiagnostic{
			...
			Severity: NetworkDiagnosticInfo,
...
func analyzeRouteConflicts(targets []netip.Prefix, existing []hostRoute) []NetworkDiagnostic {
	issues := make([]NetworkDiagnostic, 0)
	...
	for _, target := range targets {
		...
		for _, route := range existing {
			...
			// A less-specific route (including the default route) loses to the
			// precise route kube-loop installs and is therefore not a conflict.
			...
			issues = append(issues, NetworkDiagnostic{
				Code:      "route_overlap",
